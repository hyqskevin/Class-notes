...
void AES::Cipher(char *input, char *output)//加密
{
...
void AES::SubBytes(unsigned char state[][4])//通过非线性的替换函数，用查找表的方式把每个字节替换成对应的字节
{
	int r, c;
	for (r = 0; r<4; r++)
	{
...
void AES::ShiftRows(unsigned char state[][4])//矩阵中的每个横列进行循环式移位
{
	unsigned char t[4];
	int r, c;
	for (r = 1; r<4; r++)
...
void AES::MixColumns(unsigned char state[][4])//充分混合矩阵中各个直行的操作。这个步骤使用线性转换来混合每列的四个字节
{
	unsigned char t[4];
	int r, c;
	for (c = 0; c< 4; c++)
...
void AES::AddRoundKey(unsigned char state[][4], unsigned char k[][4])//矩阵中的每一个字节都与该次轮秘钥（round key）做XOR运算
{
	int r, c;
	for (c = 0; c<4; c++)
	{
...
int main(int argc, char *argv[])
{
	...
	AES aes(key);
	aes.Cipher(minwen, miwen);
	printf("%s\n", miwen);
	
	aes.InvCipher(miwen, result);
	system("pause");
	printf("解密：");
	printf("%s\n", result);
	system("pause");
